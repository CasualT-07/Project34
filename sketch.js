...
function preload()
{
  ...
  star_slots = loadImage('empty.png');
  one_star = loadImage('one_star.png');
  two_stars = loadImage('stars.png');
...
function setup() 
{
  ...
  //star slots
  slots = createSprite(100,50);
  slots.addImage(star_slots);
  slots.scale = 0.25;

  //stars
  star0 = createSprite(280,50);
  star0.addImage(star);
  star0.scale = 0.02;

  star1 = createSprite(80,400);
  star1.addImage(star);
  star1.scale = 0.02;
  
  //blower
  blower = createImg('baloon2.png');
  blower.position(250,400);
  blower.size(100,100);
...
function draw() 
{
  ...
  if(star0 != null && collide(fruit, star0)==true){
    ...
    if(stars_collected == 0) {
      stars_collected = 1;
      slots.addImage(one_star);
    } else {
      stars_collected = 2;
      slots.addImage(two_stars);
    }
  }

  if(star1 != null && collide(fruit, star1)==true){
